...
def read_redis_data(files_path):
    # Nhận danh sách các files_path và trả lại giá trị theo thứ tự
    main_result = ''
    for file in files_path:
        text = redis_db.get(file)
        main_result+=text.decode()
    return main_result
    
def transcript_audio(file_path, index):
        file = open(file_path, "rb")
        trans = openai.Audio.transcribe("whisper-1", file)
        # redis_db.set(f'{file_path}', f'PART{index}_'+trans.text)  # Hàm test đọc thứ tự redis
        redis_db.set(f'{file_path}', trans.text)

def transcripts_all_audios_parallel(files_path):
    thread_list = []
    for index, file_path in enumerate(files_path):
        t = threading.Thread(target=transcript_audio, args=(file_path,index))
        t.start() 
...
def transcribe_with_cut_file(audio, api_key):
    ...
    try:
        ...
        files_path = glob.glob(os.path.join(f'./audio/{name_folder}/', 'part_*'))
        thread_list = transcripts_all_audios_parallel(files_path)
